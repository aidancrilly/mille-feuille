...
from .surrogate import SingleFidelityGPSurrogate, MultiFidelityGPSurrogate
from .surrogate import DEFAULT_NOISE_INTERVAL, DEFAULT_LENGTHSCALE_INTERVAL
...
def evaluate_GP_scores_multi_fid(surrogate, state, Xs_test, Ys_test, Ss_test, verbose=True):
    ...
    outputs = {}
    results = []
    for label, mask in zip(["low", "high"], [0, 1]):
        ...
        if verbose:
            print(f"Fidelity {label}: R2={outputs[label]['R2']:.3f}, "
...
def plot_predictions(ax_pred, ax_resid, metrics_dict, title_suffix=""):
    ...
    for label, (y_true, y_pred, y_std, metrics) in metrics_dict.items():
        ...
        # Predictions plot
        ax_pred.errorbar(y_true, y_pred, yerr=y_std, fmt='o', alpha=0.3, c=color,
            label=f"{label} fidelity (R²={metrics['R2']:.3f})")
        # Residuals plot
        resid = y_true - y_pred
        ax_resid.hist(resid, bins=30, alpha=0.5, color=color,
            label=f"{label} fidelity (MAE={metrics['MAE']:.3f})")
    ...
    ax_pred.set_xlabel("Truth"); ax_pred.set_ylabel("Prediction")
    ax_pred.set_title(f"Truth vs Prediction {title_suffix}"); ax_pred.legend(); ax_pred.grid()
    
    ax_resid.set_xlabel("Residual"); ax_resid.set_ylabel("Frequency")
    ax_resid.set_title(f"Residuals {title_suffix}"); ax_resid.legend(); ax_resid.grid()

# ------------------------------
# k-fold evaluation
# ------------------------------

def kfold_GP_evaluation(state, k_folds=5, lengthscale_interval=DEFAULT_LENGTHSCALE_INTERVAL, 
                        noise_interval=DEFAULT_NOISE_INTERVAL, fname=None, **kernel_kwargs):
    ...
    for i, (train_idx, test_idx) in enumerate(kf.split(Xs)):
        ...
        # Prepare surrogate
        state_new = State(state.input_domain, index=I_train, Xs=X_train, Ys=Y_train,
            Ss=Ss[train_idx] if is_multi else None, fidelity_domain=state.fidelity_domain)
        SurrogateClass = MultiFidelityGPSurrogate if is_multi else SingleFidelityGPSurrogate
        surrogate = SurrogateClass(lengthscale_interval=lengthscale_interval, noise_interval=noise_interval, **kernel_kwargs)
        
        elapsed = timeit.timeit(lambda: surrogate.fit(state_new), number=1)
...
def evaluate_GP_surrogate(state, k_folds=5, lengthscale_interval=DEFAULT_LENGTHSCALE_INTERVAL, 
        noise_interval=DEFAULT_NOISE_INTERVAL, fname=None, **kernel_kwargs):
    ...
    # Run k-fold CV
    cv_metrics = kfold_GP_evaluation(state, k_folds=k_folds, lengthscale_interval=lengthscale_interval,
        noise_interval=noise_interval, fname=fname, **kernel_kwargs)
