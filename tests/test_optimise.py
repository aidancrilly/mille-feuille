...
from .conftest import ForresterDomain, PythonForresterFunction, TEST_NUM_RESTARTS, TEST_RAW_SAMPLES
...
def test_optimise_singlefidelity_GP(singlefidelitysample, batch_size, generate_acq_function):
    ...
    X_next = suggest_next_locations(batch_size, state, surrogate, acq_function, num_restarts=TEST_NUM_RESTARTS,raw_samples=TEST_RAW_SAMPLES)
    ...
    new_state = run_Bayesian_optimiser(1, batch_size, generate_acq_function, initial_state, surrogate, f, num_restarts=TEST_NUM_RESTARTS,raw_samples=TEST_RAW_SAMPLES)
